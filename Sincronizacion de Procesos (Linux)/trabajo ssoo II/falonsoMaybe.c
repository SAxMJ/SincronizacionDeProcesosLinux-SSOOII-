...
int main(int argc, char *argv[]) {
  ...
  int nMax,nMin;
  int veloC;
  //cada semaforo tendrá su numero
  int nSem;
  ...
  if((semaforo=semget(IPC_PRIVATE, nCoches+1, IPC_CREAT | 0600))==-1){
  ...
  while(pzona[350]) {
	...
	while(pzona[sig]==' ' && pzona[comprobar_cambio_carril(carril,desp)]!=' ' ){
	...
	while(pzona[comprobar_cambio_carril(carril,desp)]==' ' && pzona[sig]!= ' '){
